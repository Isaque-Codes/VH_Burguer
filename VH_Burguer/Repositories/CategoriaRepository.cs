...
namespace VH_Burguer.Repositories
{
    public class CategoriaRepository : ICategoriaRepository
    {
        ...
        {
            // ASQUERYABLE: Cria consulta sem executar nada no banco
            var consulta = _context.Categoria.AsQueryable();

            // Se for informado um ID atual
            // Edita-se uma categoria existente
            // Esta categoria será ignorada na verificação
            if (categoriaIdAtual.HasValue)
            {
                // Remove da busca a categoria com o mesmo ID
                // Evita que o sistema considere o próprio registro como duplicata
                consulta = consulta.Where(categoria => categoria.CategoriaID != categoriaIdAtual.Value);
            }

            // Verifica se existe categoria com mesmo nome e retorna true caso encontre
